...
export const getSwitchRow = (row: PixelChoice) => (): void => {
  setInterstate((state) => {
    ...
    const colorForAlternativeForChoiceEntry = createColorForAlternativeForChoiceEntry(row);

    return {
      ...colorForAlternativeForChoiceEntry,
      [rememberActiveChoiceKey]: row,
    };
  });
};
